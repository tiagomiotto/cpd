...
int main(int argc, char **argv)
{
    ...
    setenv("OMP_THREAD_LIMIT", "64", true);
    setenv("OMP_STACKSIZE", "80000",true);
...
/*
    ...
    for (i = 0; i < size * size; i++)
        matrix[i] = (int *)malloc(size * size * sizeof(int));
    for (i = 0; i < size * size; i++)
    {
...
int bactrack(int **puzzle, int size)
{
    ...
    for (i = 0; i < size; i++)
        stable[i] = (int *)malloc(size * sizeof(int));
...
#pragma omp flush(threads)
                        ...
                        for (int i1 = 0; i1 < size; i1++)
                            matrix[i1] = (int *)malloc(size * size * sizeof(int));
...
#pragma omp flush(threads)
                ...
                while (puzzle[i][j] == 0)
                {
                    ...
                    if (found == 0)
                    {
                        ...
                        {
                            printf("Thread %d: This puzzle has no solution!!\n", omp_get_thread_num());
#pragma omp critical
                            {
                                threads++;
